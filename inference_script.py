...
from src.predictors.ultralytics_predictor import UltralyticsPredictor
from src.predictors.rddc2020_predictor import Rddc2020Predictor
from src.predictors.megvii_predictor import MegviiPredictor
from ensemble_boxes import weighted_boxes_fusion, nms, non_maximum_weighted, soft_nms

def download_models():
    print("Downloading models using gdown...")
    # url = 'https://drive.google.com/uc?id=1RG2snvGmuM4KR0wC4OSSmI9rwGCqkQCr'
    ...
    # url = 'https://drive.google.com/uc?id=1rvWD0rLVwUQ4sjW3Agg1TxPdhLubvZwu'
    # output_zip = './models2.zip'
    # subprocess.run(['gdown', url, '--output', output_zip], check=True)
    # shutil.unpack_archive(output_zip, './models2')
    # os.remove(output_zip)
...
if __name__ == "__main__":
    if len(sys.argv) != 4:
        print("Usage: python inference_script.py <yaml_file> <images_path> <output_csv>")
        sys.exit(1)
